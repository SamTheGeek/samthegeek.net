...
- The gallery masonry moved from CSS multi-column to a CSS Grid, so a tile can be two columns wide without forcing a full-width break in the flow. Rows are a fine slice of one column's width (`--row-unit`, 1/200th) and each tile's height is a row span computed at build time from its aspect ratio; `container-type: inline-size` on `.gallery-page` is what lets that unit be expressed as a length. `grid-auto-flow: row dense` fills the space beside a wide tile. Tiles carry the gap as their own padding and rows are gapless, which is what makes the span arithmetic come out; the leftover quantisation is under half a percent of a tile's height and `object-fit: cover` absorbs it.
- Panoramas (any photo at 2:1 or wider) get a tile two columns wide, flush with the column grid, with ordinary photos flowing in beside them: 2 of 3 columns on desktop, the full width at the two- and one-column breakpoints. There are four - Italy has two, Canada and Japan one each - and the widest ordinary photo is 16:9, so the 2:1 cut-off separates them cleanly.
- The old first-row reordering script in `Gallery.astro` is gone; dense grid packing does that job now.
